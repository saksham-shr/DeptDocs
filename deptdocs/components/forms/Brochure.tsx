...
export default function Brochure({ data, onUpdate, onNext }: any) {
    const [isProcessing, setIsProcessing] = useState(false);
    const [dragActive, setDragActive] = useState(false);

    // Initialize files array from state or empty
    const brochureFiles = data.brochureFiles || [];
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement> | React.DragEvent) => {
        ...
        try {
            ...
            onUpdate({ brochureFiles: [...brochureFiles, ...processedFiles] });
    ...
    const removeFile = (id: string) => {
        onUpdate({ brochureFiles: brochureFiles.filter((f: any) => f.id !== id) });
    };
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500 max-w-2xl">
            ...
            <div className="space-y-6">
                <div>
                    <label htmlFor="brochure-upload" className="text-sm font-bold text-gray-700 cursor-pointer">
                ...
                {brochureFiles.length > 0 && (
                    <div className="space-y-3 pt-4 border-t border-gray-100">
                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest pb-2">Attached Brochures</h4>
                        <div className="grid grid-cols-1 gap-3">
                            {brochureFiles.map((file: any) => (
                                <div key={file.id} className="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
                                    <div className="flex items-center space-x-3 overflow-hidden">
                                        <div className="p-2 bg-gray-50 rounded-lg">
                                            {file.type === 'pdf' ? <FileText size={20} className="text-red-500" /> : <ImageIcon size={20} className="text-blue-500" />}
                                        </div>
            ...
            <div className="flex justify-end pt-6">
                <button
                    onClick={onNext}
                    className="bg-[#4F65F6] text-white px-10 py-3 rounded-lg font-bold flex items-center space-x-2 hover:bg-[#3d50c2] transition-all"
                >
                    <span>Next</span>
