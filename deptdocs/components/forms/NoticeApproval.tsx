...
import { ChevronRight, Upload, X, FileCheck, FileText, Image as ImageIcon } from 'lucide-react';

export default function NoticeApproval({ data, onUpdate, onNext }: any) {
    const [isProcessing, setIsProcessing] = useState(false);
    const [dragActive, setDragActive] = useState(false);

    // Initialize files array from state or empty
    const approvalFiles = data.approvalFiles || [];
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement> | React.DragEvent) => {
        ...
        try {
            const processedFiles = await Promise.all(selectedFiles.map(async (file) => {
                ...
                // CASE 2: PDF (Convert pages to Base64 images - DYNAMICALLY IMPORTED)
                    ...
                    for (let i = 1; i <= pdfDoc.numPages; i++) {
                        ...
                        // TypeScript bypass for render parameters
                        const renderContext = {
            ...
            onUpdate({ approvalFiles: [...approvalFiles, ...processedFiles] });
    ...
    const removeFile = (id: string) => {
        onUpdate({ approvalFiles: approvalFiles.filter((f: any) => f.id !== id) });
    };

    return (
        <div className="space-y-10 animate-in fade-in duration-500 max-w-2xl">
            <div className="flex flex-col">
                <h3 className="text-xl font-bold text-[#1a365d] border-b-2 border-[#1a365d] inline-block pb-1 self-start">
                    Notice for Approval
                </h3>
            </div>
            ...
            <div className="space-y-6">
                <div>
                    <label htmlFor="notice-upload" className="text-sm font-bold text-gray-700 cursor-pointer">
                ...
                {approvalFiles.length > 0 && (
                    <div className="space-y-3 pt-4 border-t border-gray-100">
                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest pb-2">Attached Notices</h4>
                        <div className="grid grid-cols-1 gap-3">
                            {approvalFiles.map((file: any) => (
                                    ...
                                    <div className="flex items-center space-x-3 overflow-hidden">
                                        <div className="p-2 bg-gray-50 rounded-lg">
                                            {file.type === 'pdf' ? (
                                                ...
                                                <FileCheck size={20} className="text-[#3b5998]" />
                                            )}
                                        </div>
                                        <div className="flex flex-col">
